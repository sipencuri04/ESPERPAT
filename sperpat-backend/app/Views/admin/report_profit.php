...
<div class="admin-layout">
    <aside class="admin-sidebar">
        ...
        <ul class="sidebar-menu">
            ...
            <li><a href="<?= base_url('/admin/reports/profit') ?>" class="active"><i class="bi bi-currency-dollar"></i> Lap. Laba</a></li>
            <li><a href="<?= base_url('/admin/reports/stock') ?>"><i class="bi bi-archive"></i> Lap. Stok</a></li>
        </ul>
    </aside>
    
    <div class="admin-content">
        <h3 class="fw-800 mb-4">Laporan Laba</h3>
        
        <div class="card-dark mb-4">
            <form action="<?= base_url('/admin/reports/profit') ?>" method="get" class="form-dark">
        ...
        <div class="row g-4 mb-4">
            ...
            <div class="col-md-4">
                <div class="stat-card">
                    <div class="stat-value" style="color:var(--success);">Rp <?= number_format($totalProfit ?? 0, 0, ',', '.') ?></div>
                    <div class="stat-label">Total Laba</div>
                </div>
            </div>
            <div class="col-md-4">
                <div class="stat-card">
                    <div class="stat-value"><?= $totalSales > 0 ? round(($totalProfit / $totalSales) * 100, 1) : 0 ?>%</div>
        ...
        <div class="card-dark">
            <div class="table-responsive table-dark-custom">
                <table class="table table-hover">
                    <thead>
                        <tr><th>Invoice</th><th>Customer</th><th>Total</th><th>Laba</th><th>Tanggal</th></tr>
                    </thead>
                    <tbody>
                        <?php if (!empty($orders)): ?>
                            <?php foreach ($orders as $order): ?>
                                <tr>
                                    ...
                                    <td style="color:var(--success);" class="fw-700">Rp <?= number_format($order['profit'] ?? 0, 0, ',', '.') ?></td>
                                    <td class="text-secondary"><?= date('d/m/Y', strtotime($order['created_at'])) ?></td>
                                </tr>
                            <?php endforeach; ?>
                        <?php else: ?>
                            <tr><td colspan="5" class="text-center text-secondary">Tidak ada data</td></tr>
                        <?php endif; ?>
                    </tbody>
                </table>
            </div>
