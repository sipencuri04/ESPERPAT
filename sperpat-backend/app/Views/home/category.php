<?php
// Category Explorer - DROPS 2026 Aesthetic
?>
...
<div class="container py-4">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4 pt-2 px-1">
        ...
        <h6 class="fw-800 mb-0"><?= strtoupper(esc($category['name'])) ?></h6>
    ...
    <div class="row g-4">
            ...
            ?>
                <div class="col-6">
                    <div class="product-card-drops" onclick="window.location.href='<?= base_url('/produk/' . ($product['slug'] ?? '')) ?>'">
                        ...
                        <div class="card-info">
                            <h6 class="card-title-drops"><?= esc($product['name']) ?></h6>
        ...
        <?php else: ?>
            <div class="col-12 text-center py-5 mt-5">
                <i class="bi bi-box-seam display-1 text-light mb-4"></i>
                <h4 class="fw-900">No components found</h4>
                <p class="text-muted">Currently stocking this category. Check back soon!</p>
            </div>
        <?php endif; ?>
    </div>
</div>
...
<div class="ultra-bottom-sheet" id="filterPanel">
    ...
    <div class="p-4">
        <div class="d-flex justify-content-between align-items-center mb-5">
            <h4 class="fw-900 mb-0">Sort Configuration</h4>
            <button class="btn-close-circle" id="btnCloseFilter"><i class="bi bi-x"></i></button>
        </div>
        
        <form action="<?= base_url('/kategori/' . ($category['slug'] ?? '')) ?>" method="get">
            <div class="mb-5">
                <div class="d-grid gap-3">
                    <button type="submit" name="sort" value="latest" class="filter-option-btn">New Arrivals</button>
                    <button type="submit" name="sort" value="price_low" class="filter-option-btn">Lowest Investment</button>
                    <button type="submit" name="sort" value="price_high" class="filter-option-btn">Premium Tiers</button>
                </div>
            </div>
...
.sheet-handle { width: 40px; height: 5px; background: #eee; border-radius: 10px; margin: 15px auto; }
.btn-close-circle { width: 44px; height: 44px; border-radius: 50%; border: none; background: #f8f8f8; display: flex; align-items: center; justify-content: center; }
